...
func _ready():
	if h_xr:
		...
		$CanvasLayer/Name.text = h_name
		$CanvasLayer/IP.text = h_ip
		$CanvasLayer/Port.text = h_port
		$CanvasLayer/OptionButton.select(h_mapid)
...
func update_scores():
	var text = ""
	for i in get_children():
			if i.name != "CanvasLayer" and i.name != "Track" and i.name != "MultiplayerSpawner" and i.name != "Countdown" and i.name != "XROrigin3D":
